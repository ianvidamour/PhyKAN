...
model = PhyKAN(shape, nfilt, thresholding=thresholding)
Ntr = 10000
Ncheck = 1000
saved_weights = []
saved_thresholds = []
...
for run in range(10):
    ...
    np.save('KIN8NM No prune Accuracies, N'+str(N)+', nfilt'+str(nfilt)+', run'+str(run)+'.npy', np.asarray(accuracies))
    np.save('KIN8NM No prune Filter Params, N'+str(N)+', nfilt'+str(nfilt)+', run'+str(run)+'.npy', np.asarray(saved_weights, dtype='object'))
    np.save('KIN8NM No prune Threshold Params, N'+str(N)+', nfilt'+str(nfilt)+', run'+str(run)+'.npy', np.asarray(saved_thresholds, dtype='object'))
