...
Netsizes = [5, 10, 15, 25, 50, 75, 100, 125, 150, 200]
...
for N in Netsizes:
    for run in range(5, 10):
        ...
        for i in range(Ntr):
            ...
            true_loc = model2.forward_model(pred)
            loss = lossfn(true_loc, Xs)
            loss.backward()
            optimiser.step()
            if i%Ncheck == 0:
                with torch.no_grad():
                    prediction = model.forward(x_val)
                    true_location = model2.forward_model(prediction)
                loss = lossfn(true_location, x_val)
                print(loss.item())
        
        torch.save(model, 'Inverse Kinematics MLP augmented, N='+str(N)+', run'+str(run)+'.pt')
