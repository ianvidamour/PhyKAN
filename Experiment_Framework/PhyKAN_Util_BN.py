...
class PhyKAN(nn.Module):
    def __init__(self, KANshape, filters_per_unit, low_pass_lookup, high_pass_lookup, thresholding=False, lr_decay=0.99, sigmoid_s=0.5, lr=1e-3, batch_norm=True):
        super().__init__()
        ...
        for i in range(self.Nlayers):
            ...
            gains = (torch.rand((KANshape[i], KANshape[i+1], filters_per_unit, 1)) - 0.5 ) /filters_per_unit
            ...
            self.norm_biases.append(torch.zeros((1, KANshape[i+1]), requires_grad=True))
            self.norm_scalings.append(torch.ones((1, KANshape[i+1]), requires_grad=True))
            self.norm_moving_avgs.append(torch.zeros((KANshape[i+1], 2)))
        ...
        self.batch_optim = torch.optim.Adam(params=self.norm_biases+self.norm_scalings, lr=lr)
    ...
    def batch_normalisation(self, Xin, l, training=False):
        if training==True:
            # Find mean and standard deviation along batch
            mu = Xin.mean(dim=0)
            sigma = Xin.std(dim=0)
            # Update running averages
            self.norm_moving_avgs[l][:, 0] = self.norm_moving_avgs[l][:, 0]*0.95 + 0.05*mu
            self.norm_moving_avgs[l][:, 1] = self.norm_moving_avgs[l][:, 1]*0.95 + 0.05*sigma
            # Normalise layer
            Xin_norm = (Xin-mu)/(sigma+1e-3)
            # Scale and shift
            Xout = Xin_norm * self.norm_scalings[l] + self.norm_biases[l]
        if training==False:
            # Normalise layer
            Xin_norm = (Xin-self.norm_moving_avgs[l][:, 0])/(self.norm_moving_avgs[l][:, 1]+1e-3)
            # Scale and shift
            Xout = Xin_norm * self.norm_scalings[l] + self.norm_biases[l]
        return Xout
        
    
    def forward(self, Xin, discrete=False, batch_norm=False, training=False):
        ...
        if discrete==True:
            ...
            for layer in range(self.Nlayers):
                layer_outs = self.band_pass_discrete(inputs[layer], self.filter_params[layer], self.edge_mask[layer])
                if batch_norm == True:
                    outputs[layer][:, :] = self.batch_normalisation(layer_outs, layer, training)
                else:
                    outputs[layer][:, :] = layer_outs
            ...
            # Pass through model
            for layer in range(self.Nlayers):
                layer_outs = self.band_pass_continuous(inputs[layer], self.filter_params[layer], self.edge_mask[layer])
                if batch_norm == True:
                    outputs[layer][:, :] = self.batch_normalisation(layer_outs, layer, training)
                else:
                    outputs[layer][:, :] = layer_outs
    ...
    def train(self, Xin, Yin, penalty=True, stability_penalty=True, lamda=1e-4, stablam=1e-8, discrete=False):
        # Reset optimisers
        self.filter_optim.zero_grad()
        self.threshold_optim.zero_grad()
        self.batch_optim.zero_grad()
        # Pass through Model
        activations = self.forward(Xin, discrete=discrete, training=True)
        pred = activations[-1]
        # Calculate loss
        ...
        if stability_penalty==True:
            ...
            loss.backward()
            stability_loss = torch.tensor([0])
        # Update parameters
        self.filter_optim.step()
        self.threshold_optim.step()
        self.batch_optim.step()

        return activations, loss.item(), loss_np.item(), stability_loss.item()
