...
class PhyKAN_critic(nn.Module):
    ...
    def train_ik(self, Xin, Yin, penalty=True, stability_penalty=True, lamda=1e-4, stablam=1e-5, discrete=False):
        ...
        activations = self.forward(Xin, discrete=discrete)
        pred = activations[-1]
        true_loc = self.load_model.forward(pred)[-1]
        # Calculate loss
        loss_np = self.lossfn(true_loc, Yin)
        # Add penalty terms
        if penalty==True:
            penalty_loss = lamda * self.penalty(activations)
            loss = loss_np + penalty_loss
...
class PhyKAN_actor(nn.Module):
    ...
    def train_actor_critic(self, Xin, penalty=True, stability_penalty=True, lamda=1e-4, stablam=1e-5, discrete=False):
        ...
        loss = -1*q_values.mean() + 1e-4*actions.abs().mean()
        loss.backward()
        # Update parameters
        self.filter_optim.step()
        self.threshold_optim.step()
