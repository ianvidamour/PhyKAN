...
shape = [3, N, N, 6]
forward_model= torch.load('./HPC_Data/0.1/Noised Forward Kinematics MLP 2L, N='+str(N)+' run = '+str(run)+', noise_k='+str(noise_k)+'.pt', weights_only=False, map_location=device)
...
torch.save(model, './HPC_Data/0.1/Noised Inverse Kinematics Model MLP 2L, N='+str(N)+' run '+str(run)+', noise_k='+str(noise_k)+'.pt')
np.save('./HPC_Data/0.1/Noised Inverse Training Accuracies MLP 2L, N='+str(N)+' run '+str(run)+', noise_k='+str(noise_k)+'.npy', accuracies)
