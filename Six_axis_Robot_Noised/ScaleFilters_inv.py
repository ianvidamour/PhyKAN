...
for run in range(10):
    for N in Netsizes:
        edge_data = np.load(transfer_folder+f'Inv Kin 2L Chirped Transfer, N{N}, {run}.npy', allow_pickle=True)
        ...
        arr_obj[:] = data
        np.save(transfer_folder+'Noised Inv Kin Scaled Filters 2L, N '+str(N)+' Run '+str(run)+'.npy', arr_obj, allow_pickle=True)
