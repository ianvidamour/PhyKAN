...
accuracies_out = np.zeros((12, 10))
parameters_out = np.zeros((12, 10))
dims_out_2l = np.zeros((12, 10, 4))
...
for i, N in enumerate(Netsizes):
    for run in range(10):
        model = torch.load(root_folder+subfolder+'Noised Forward Kinematics Model 2L, N='+str(N)+' run '+str(run)+', noise_k='+str(noise_k)+'.pt', weights_only=False, map_location='cpu')
...
for i, N in enumerate(Netsizes):
    for run in range(10):
        model_2l = torch.load(root_folder+subfolder+'Noised Forward Kinematics MLP 2L, N='+str(N)+' run = '+str(run)+', noise_k='+str(noise_k)+'.pt', weights_only=False, map_location='cpu')
        pred_2l = model_2l.forward(x_test)
        loss = lossfn(pred_2l, y_test)
        accuracies_outMLP[i, run] = loss.item()
        parameters_outMLP[i, run] = trainables_MLP_2L(N)
        l1_acts = model_2l.fc1(x_test).relu()
        l2_acts = model_2l.fc2(l1_acts).relu()
        l3_acts = model_2l.fc3(l2_acts)
...
axes[0].set_title('Forward Kinematics 2L, Hidden Layer 1')
...
fig, axes = plt.subplots(2,1, figsize=(5,6))
axes[0].set_title('Forward Kinematics 2L, Hidden Layer 2')
