...
Netsizes = [2, 3, 4, 5, 7, 9, 10, 12, 15, 20, 25, 30]
subfolder = '/'+str(noise_k)+'/'
accuracies_out2 = np.zeros((12, 10))
parameters_out2 = np.zeros((12, 10))
for i, N in enumerate(Netsizes):
    for run in range(10):
        model_KAN = torch.load(path+data_folder+subfolder+'Noised Forward Kinematics Model 2L, N='+str(N)+' run '+str(run)+', noise_k='+str(noise_k)+'.pt', weights_only=False, map_location='cpu')
        model_pred = model_KAN.forward(x_test, input_sigmoid=False)[-1]
        accuracy = loss_function(model_pred, y_test)
        parameters_out2[i, run] = edges_KAN_2L(N)
        accuracies_out2[i, run] = accuracy.item()
np.save('Forward Kinematics PhyKAN 2L Simulation Accuracies noise_k=0.1.npy', accuracies_out2)
np.save('Forward Parameter Counts, PhyKAN 2L.npy', parameters_out2)
...
for i, N in enumerate(Netsizes):
    for run in range(10):
        model = torch.load(path+data_folder+subfolder+'Noised Forward Kinematics MLP 2L, N='+str(N)+' run = '+str(run)+', noise_k='+str(noise_k)+'.pt', weights_only=False, map_location='cpu')
        model_pred = model.forward(x_test)
        accuracy = loss_function(model_pred, y_test)
        parameters_outMLP2L[i, run] = edges_MLP_2L(N)
