...
class PhyKAN(nn.Module):
    def __init__(self, KANshape, filters_per_unit, low_pass_lookup, high_pass_lookup, thresholding=False, lr_decay=0.99, sigmoid_s=0.5, lr=1e-3, batch_norm=True):
        ...
        for i in range(self.Nlayers):
            ...
            gains = torch.randn((KANshape[i], KANshape[i+1], filters_per_unit, 1))/filters_per_unit
    ...
    def forward(self, Xin, discrete=False, batch_norm=True, training=False):
        batch_size = len(Xin)
    ...
    def train(self, Xin, Yin, penalty=True, stability_penalty=True, lamda=1e-4, stablam=1e-8, discrete=False):
        ...
        # Pass through Model
        activations = self.forward(Xin, discrete=discrete, training=True, batch_norm=self.batch_norm)
        pred = activations[-1]
        # Calculate loss
