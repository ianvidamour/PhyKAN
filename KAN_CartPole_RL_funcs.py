...
def step_agent_softmax(state, model, softmax_tau=1):
    ...
    # Predict Q values
    output = model.forward(torch.from_numpy(inputs))[-1]
...
def training_step(model, prediction_network, replay_buffer, Nactions, Nstates, batch_size=500):
    ...
    ks = np.random.choice(np.arange(0, len(replay_buffer), 1, dtype='int'), np.min([batch_size, len(replay_buffer)]))
    ...
    q_values = model.forward(current_inputs)[-1]
