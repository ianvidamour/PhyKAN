...
Plots here evaluate the ability to fit 'True' Locations from un-noised joint angles
'''
...
prenoise_joint_angles = np.load(path+input_folder+'PreNoise_6a_joint_angles_noise_k='+str(noise_k)+'.npy')
...
Netsizes = [2, 3, 4, 5, 7, 9, 10]
accuracies_out2 = np.zeros((7, 10))
parameters_out2 = np.zeros((7, 10))
for i, N in enumerate(Netsizes):
    for run in range(10):
        model_KAN = torch.load(path+data_folder+'Noised Forward Kinematics Model Memristor 2L, N='+str(N)+', run '+str(run)+', noise_k='+str(noise_k)+'.pt', weights_only=False, map_location='cpu')
...
np.save('Forward Kinematics MemKAN 2L Simulation Accuracies noise_k=0.1.npy', accuracies_out2)
np.save('Forward Parameter Counts, MemKAN 2L.npy', parameters_out2)
...
for i, N in enumerate(Netsizes):
    for run in range(10):
        model = torch.load(path+data_folder+'Noised Forward Kinematics MLP 2L, N='+str(N)+' run = '+str(run)+', noise_k='+str(noise_k)+'.pt', weights_only=False, map_location='cpu')
        model_pred = model.forward(x_test)
        accuracy = loss_function(model_pred, y_test)
        parameters_outMLP2L[i, run] = trainables_MLP_2L(N)
        accuracies_outMLP2L[i, run] = accuracy.item()
np.save('Forward Kinematics MLP 2L Accuracies noise_k=0.1.npy', accuracies_outMLP2L)
np.save('Forward Parameter Counts, MLP 2L.npy', parameters_outMLP2L)
...
plt.loglog(parameters_out2.mean(axis=1), stats.gmean(accuracies_out2, axis=1), color='red', label='Software MemKAN', marker='o')
