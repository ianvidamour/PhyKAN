...
for run in range(10):
    ...
    torch.save(model, 'Inverse Kinematics Model MLP 1L, N='+str(N)+' run '+str(run)+'.pt')
    np.save('Inverse Training Accuracies MLP 1L, N='+str(N)+' run '+str(run)+'.npy', accuracies)
    np.save('Intrinsic Dims MLP 1L, N='+str(N)+' run '+str(run)+'.npy', intrinsic_dims)
