...
actor_model = PhyKAN_actor(actor_shape, 6, low_vals, high_vals, load_model=critic_model, lr=1e-3)
prediction_network = PhyKAN_critic(critic_shape, 6, low_vals, high_vals, lr=1e-2)
for l in range(critic_model.Nlayers):
    prediction_network.filter_params[l] = torch.clone(critic_model.filter_params[l])
...
for episode in range(1000):
    ...
    for step in range(500):
        ...
        if upright == True:
            ...
            # Step critic model
            loss, rawloss, stabloss = critic_model.train(current_inputs.float(), targets.float(), penalty=True, lamda=1e-4)
            # Soft update for prediction network
            with torch.no_grad():
                for l in range(prediction_network.Nlayers):
                    prediction_network.filter_params[l] = prediction_network.filter_params[l] * (1-update_tau) + update_tau*critic_model.filter_params[l]
            # step actor model
