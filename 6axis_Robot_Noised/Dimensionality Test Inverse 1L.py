...
for i, N in enumerate(Netsizes):
    for run in range(10):
        model = torch.load(root_folder+subfolder+'Noised Inverse Kinematics Model 1L, N='+str(N)+' run '+str(run)+', noise_k='+str(noise_k)+'.pt', weights_only=False, map_location='cpu')
...
for i, N in enumerate(Netsizes):
    for run in range(10):
        model_1l = torch.load(root_folder+subfolder+'Noised Inverse Kinematics Model MLP 1L, N='+str(N)+' run '+str(run)+', noise_k='+str(noise_k)+'.pt', weights_only=False, map_location='cpu')
        pred_1l = model_1l.forward(x_test)
        locs = model.fw_kin(pred_1l)
        accuracies_outMLP[i, run] = lossfn(locs, x_test)
...
axes[0].set_title('Inverse Kinematics 1L, Hidden Layer')
