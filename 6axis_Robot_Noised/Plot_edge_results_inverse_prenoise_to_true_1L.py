...
def edges_KAN_1L(N):
    # Edges = (in * hidden + hidden * hidden + hidden * out)
    return (6 * N + N * 3)


def edges_MLP_1L(Netsize):
    # Edges = in * hidden + hidden * hidden + hidden * out (weights)
    return (6 * N + N * 3)
...
for i, N in enumerate(Netsizes):
    for run in range(10):
        model_KAN = torch.load(path+data_folder+subfolder+'Noised Inverse Kinematics Model 1L, N='+str(N)+' run '+str(run)+', noise_k='+str(noise_k)+'.pt', weights_only=False, map_location='cpu')
...
np.save('Inverse Kinematics PhyKAN 1L Simulation Accuracies noise_k=0.1.npy', accuracies_out2)
np.save('Inverse Parameter Counts, PhyKAN 1L.npy', parameters_out2)
...
for i, N in enumerate(Netsizes):
    for run in range(10):
        model = torch.load(path+data_folder+subfolder+'Noised Inverse Kinematics Model MLP 1L, N='+str(N)+' run '+str(run)+', noise_k='+str(noise_k)+'.pt', weights_only=False, map_location='cpu')
...
np.save('Inverse Kinematics MLP Accuracies 1L noise_k=0.1.npy', accuracies_outMLP2L)
np.save('Inverse Parameter Counts, MLP 1L.npy', parameters_outMLP2L)
        

        
import scipy.stats as stats
plt.figure(dpi=1200, figsize=(4,3))
plt.title('Inverse 1L, Prenoised to true (Noise k = '+str(noise_k)+')', fontsize=14)
