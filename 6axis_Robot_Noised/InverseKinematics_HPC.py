...
forward_model = torch.load('Noised Forward Kinematics Model 1L, N='+str(N)+' run '+str(run)+', noise_k='+str(noise_k)+'.pt', weights_only=False, map_location=device)
...
torch.save(model, 'Noised Inverse Kinematics Model 1L, N='+str(N)+' run '+str(run)+', noise_k='+str(noise_k)+'.pt')
np.save('Noised Inverse Training Accuracies 1L, N='+str(N)+' run '+str(run)+', noise_k='+str(noise_k)+'.npy', accuracies)
