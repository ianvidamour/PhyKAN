...
train_labels = train_targets.to('cpu')
test_inputs = test_inputs.to('cpu')
test_labels = test_targets.to('cpu')
...
for i in range(200000):
    Xs, Ys = gen_samples(train_inputs, train_labels, 100)
...
torch.save(model, 'Trained FMNIST model full '+str(node)+'nodes, '+str(nfilt)+' filters.pt')
np.save('Model Accuracy Full '+str(node)+'nodes, '+str(nfilt)+' filters.npy', accuracy)
