...
#%%
gains = np.zeros((5104, 3, 20, 6))
low_pass = np.zeros((5104, 3, 20, 6))
high_pass = np.zeros((5104, 3, 20, 6))
for i in range(2500):
    gains[i] = saved_weights[i][0][:, :, :, 0]
    low_pass[i] = saved_weights[i][0][:, :, :, 1]
    high_pass[i] = saved_weights[i][0][:, :, :, 2]
    
plt.figure()
plt.plot(high_pass[:, 0, 8, :])
plt.show()
