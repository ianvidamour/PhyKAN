...
for run in range(10):
    ...
    for i in range(Ntr):
        ...
        loss = model.train(Xs, Ys, penalty=True, lamda=1e-6)
        if i%Ncheck == 0:
            model.sched1.step()
            print(loss)
            if i%Nprune==0:
    ...
    np.save('Inverse Kinematics Accuracies Direct, N'+str(N)+', nfilt'+str(nfilt)+', run'+str(run)+'.npy', np.asarray(accuracies))
    np.save('Inverse Kinematics Filter Params Direct, N'+str(N)+', nfilt'+str(nfilt)+', run'+str(run)+'.npy', np.asarray(saved_weights, dtype='object'))
    np.save('Inverse Kinematics  Threshold Params Direct, N'+str(N)+', nfilt'+str(nfilt)+', run'+str(run)+'.npy', np.asarray(saved_thresholds, dtype='object'))
    torch.save(model, 'Full Kinematics Model Direct, N'+str(N)+', nfilt'+str(nfilt)+', run'+str(run)+'.pt')
