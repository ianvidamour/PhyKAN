...
class PhyKAN(nn.Module):
    def __init__(self, KANshape, filters_per_unit, thresholding=False, lr_decay=0.99, sigmoid_s=0.5):
        ...
        self.sched1 = torch.optim.lr_scheduler.ExponentialLR(self.filter_optim, lr_decay)
...
N=9
model = torch.load('Forward Kinematics Model, N='+str(N)+'.pt')
Xdata = np.load('Forward Kinematics Inputs.npy')
Ydata = np.load('Forward Kinematics Targets.npy')

Xdata = torch.from_numpy(np.load('Forward Kinematics Inputs.npy')).to(device)
Ydata = torch.from_numpy(np.load('Forward Kinematics Targets.npy')).to(device)

errors = []
noise_sigmas = torch.logspace(-4, -1, 100)
for noise_sigma in noise_sigmas:
    noise = torch.randn(Xdata.shape)*noise_sigma
    with torch.no_grad():
        prediction = model.forward(Xdata+noise)
    loss = model.lossfn(Ydata, prediction)
    errors.append(loss.item())
    
import matplotlib.pyplot as plt

fig, ax = plt.subplots(figsize=(4,3), dpi=1200)
ax.set_xlabel('Joint angle uncertainty (normalised)')
ax.set_ylabel('Mean Squared Error')
ax.set_yscale('log')
ax.set_xscale('log')
ax.plot(noise_sigmas.cpu().numpy(), errors)
plt.show()
